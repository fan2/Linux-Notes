...
例1：逐行打印输出 `ifconfig -l` 中的接口。

1. 输入字段分割符FS默认为空格，然后在BEGIN中指定输出字段分隔符OFS为换行，BODY部分for循环打印各个字段，即实现了单行输出。

```
ifconfig -l | awk 'BEGIN{OFS="\n"} { for (i=1; i<=NF; i++) print $i}'
```

2. 更简单的方案：指定输入记录分隔符RS为空格，将接口列表按空格分割成多条记录，再逐行（ORS默认换行）列举打印记录：

```
ifconfig -l | awk 'BEGIN{RS=" "} {print}'
```

例2：逐行打印输出 `PATH` 中的环境变量。

1. `-F` 指定以 `:` 作为字段分割符（FS），然后在BEGIN中指定输出字段分隔符OFS为换行，BODY部分for循环打印各个字段，即实现了单行输出。

```
echo $PATH | awk -F ':' 'BEGIN{OFS="\n"} { for (i=1; i<=NF; i++) print $i}'
```

2. 更简单的方案：指定输入记录分隔符RS为 `:`，将 PATH 环境变量按冒号分割成多条记录，再逐行（ORS默认换行）列举打印记录：
